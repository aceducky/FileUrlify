...
function Form({ files, setFiles, service, setService }) {
  ...
  function validateFiles(inputFiles) {
    ...
    if (allFiles.some((file) => file.size > service.maxFileSize)) {
      alert(
        `Each file must be less than ${service.maxFileSize / (1024 * 1024)} MB`
      );
      return;
    }
    setIsValid(true);
  ...
  function handleUpload() {
    ...
    const files_to_upload = new FormData();
    files.forEach((file)=>files_to_upload.append("file", file))
    service.uploadFunction(files_to_upload);
  }
